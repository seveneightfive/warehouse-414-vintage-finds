...
interface AdminPlaceHoldDialogProps {
  open: boolean;
...
const AdminPlaceHoldDialog = ({ open, onOpenChange, productName, onConfirm, isLoading }: AdminPlaceHoldDialogProps) => {
  const [customerName, setCustomerName] = useState('');
  const [customerEmail, setCustomerEmail] = useState('');
  const [customerPhone, setCustomerPhone] = useState('');
  const [days, setDays] = useState('3');
  const [platform, setPlatform] = useState('414');
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md bg-white">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label htmlFor="hold-platform">Platform</Label>
            <Select value={platform} onValueChange={setPlatform}>
              <SelectTrigger id="hold-platform">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
...
export default AdminPlaceHoldDialog;
