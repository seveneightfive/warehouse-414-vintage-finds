...
export default function ProductActions({ product }: ProductActionsProps) {
  ...
  const submitSpecSheet = async () => {
    ...
    try {
      ...
      doc.text('HIGH STYLE FASHIONABLE VINTAGE FURNISHINGS & ART', margin, y);
      y += 4;
      ...
      const filename = `${product.sku || product.id}-spec-sheet.pdf`;
      doc.save(filename);
  ...
  const menuItems = [
    ...
    { action: 'specsheet' as ActionType, label: 'Download Spec Sheet', icon: '↓', desc: 'PDF with dimensions & details' },
  ];
  ...
  const modalConfig: Record<string, { title: string; subtitle: string; onSubmit: () => void; submitLabel: string }> = {
    inquiry: {
      ...
      subtitle: `About: ${product.name}`,
      onSubmit: submitInquiry,
      submitLabel: 'Send Question',
    },
    offer: {
      title: 'Make an Offer',
      subtitle: product.price ? `Listed at $${product.price.toLocaleString()}` : `About: ${product.name}`,
      onSubmit: submitOffer,
      submitLabel: 'Submit Offer',
    },
    purchase: {
      title: 'Purchase This Piece',
      subtitle: 'We\'ll prepare a shipping quote and invoice',
      onSubmit: submitPurchase,
      submitLabel: 'Request Invoice',
    },
    hold: {
      title: 'Place on Hold',
      subtitle: 'Reserve this item for up to 5 days',
      onSubmit: submitHold,
      submitLabel: 'Confirm Hold',
    },
    specsheet: {
      title: 'Download Spec Sheet',
      subtitle: 'Enter your details to receive the PDF',
      onSubmit: submitSpecSheet,
  ...
  return (
    <>
      ...
      <div className="relative" ref={menuRef}>
        ...
        {menuOpen && (
          <div className="absolute left-0 top-full mt-1.5 w-64 bg-card border border-border
            rounded-sm shadow-lg z-50 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
              ...
              {success ? (
                <div className="py-4 text-center space-y-3">
                  <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center mx-auto">
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                      <path d="M3 9l4.5 4.5L15 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-primary" style={{stroke: 'var(--primary)'}}/>
                    </svg>
                  </div>
                  <p className="text-sm font-display text-foreground leading-relaxed">{success}</p>
                  <button onClick={closeModal} className="text-[11px] tracking-[0.12em] uppercase text-muted-foreground hover:text-foreground transition-colors font-display mt-2">
                    Close
                  </button>
                </div>
              ) : (
                <>
                  ...
                  {activeAction === 'purchase' && (
                    <FormField label="Shipping Zip Code" value={zipCode} onChange={setZipCode} placeholder="For freight quote" required />
                  )}
                  ...
                  {activeAction === 'specsheet' && (
                    <div className="flex items-center gap-3 py-1">
                      <button
                        type="button"
                        onClick={() => setIncludePrice(prev => !prev)}
                  ...
                  {activeAction === 'hold' && (
                    <p className="text-[10px] text-muted-foreground font-display leading-relaxed border-l-2 border-primary/30 pl-3">
                      Holds are valid for 5 days. After that, the item returns to available status. We'll send a confirmation to your email.
                    </p>
                  )}
                  ...
                  {activeAction === 'purchase' && (
                    <p className="text-[10px] text-muted-foreground font-display leading-relaxed border-l-2 border-primary/30 pl-3">
                      All purchases are handled personally. We'll contact you with a freight quote and invoice tailored to your location.
                    </p>
                  )}
