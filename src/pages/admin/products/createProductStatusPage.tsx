...
import {
  ...
  Eye, Pencil, Trash2, LinkIcon, Tag, ArrowLeft,
} from 'lucide-react';
...
const statusConfig: Record<StatusValue, { title: string; subtitle: string; emoji: string; showSoldDetails?: boolean; showAuctionUrl?: boolean }> = {
  draft: { title: 'Draft Products', subtitle: 'Work in progress', emoji: '📝' },
  at_auction: { title: 'At Auction', subtitle: 'Currently listed on marketplaces', emoji: '🔨', showAuctionUrl: true },
  sold: { title: 'Sold Products', subtitle: 'Historical records', emoji: '✓', showSoldDetails: true },
  inventory: { title: 'Inventory', subtitle: 'Back stock and consignment', emoji: '📦' },
  deactivated: { title: 'Deactivated', subtitle: 'Hidden from customers', emoji: '⊘' },
};

interface CreateProductStatusPageProps {
  status: StatusValue;
}
...
export function createProductStatusPage({ status }: CreateProductStatusPageProps) {
  return function ProductStatusPage() {
    ...
    const showSoldDetails = config.showSoldDetails;
    const showAuctionUrl = config.showAuctionUrl;
    ...
    const releaseFromSoldMutation = useMutation({
      mutationFn: async (id: string) => {
        const { error } = await supabase.from('products').update({ status: 'available' } as any).eq('id', id);
        if (error) throw error;
      },
      onSuccess: () => { invalidate(); toast.success('Released back to Available'); },
      onError: (err: Error) => toast.error(err.message),
    });

    const releaseFromAuctionMutation = useMutation({
      mutationFn: async (id: string) => {
  ...
  onSuccess: () => { invalidate(); toast.success('External links saved'); setLinksProduct(null); setLinksState({ firstdibs_url: '', chairish_url: '', ebay_url: '' }); },
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-2xl tracking-wide text-foreground">{config.title}</h1>
            <p className="text-sm text-muted-foreground mt-1">{config.subtitle} — {data?.total ?? 0} items</p>
          </div>
          {status === 'draft' && (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {showAuctionUrl && <TableHead>Auction URL</TableHead>}
                  {!showSoldDetails && !showAuctionUrl && <TableHead className="w-28">Price</TableHead>}
                ...
                {products.map((p) => {
                  ...
                  return (
                    <TableRow key={p.id}>
                      ...
                      {!showSoldDetails && !showAuctionUrl && (
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-52">
                            ...
                            <DropdownMenuItem asChild>
                              <Link to={`/admin/products/${p.id}?from=${status}`} className="flex items-center gap-2 cursor-pointer">
                            ...
                            {status === 'sold' && (
                              <DropdownMenuItem
                                onClick={() => { if (confirm('Move back to Available?')) releaseFromSoldMutation.mutate(p.id); }}
                            ...
                            {status === 'at_auction' && (
                              <DropdownMenuItem
                                onClick={() => { if (confirm('Release from auction?')) releaseFromAuctionMutation.mutate(p.id); }}
                                className="flex items-center gap-2 cursor-pointer"
                              >
